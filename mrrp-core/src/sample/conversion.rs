...
// The following conversions assume `-1.0 <= s < 1.0` (note that +1.0 is
// excluded) and will overflow otherwise.
conversions!(f32, f32 {
...
macro_rules! impl_from_sample {
    ($T:ty, $fn_name:ident from $({$U:ident: $Umod:ident})*) => {
        $(
            ...
            impl FromSample<Complex<$U>> for Complex<$T> {
                #[inline]
                fn from_sample(sample: Complex<$U>) -> Self {
                    Complex {
                        re: self::$Umod::$fn_name(sample.re),
                        im: self::$Umod::$fn_name(sample.im),
                    }
                }
            }
        )*
