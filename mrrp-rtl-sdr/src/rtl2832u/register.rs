...
pub mod shadow {
    ...
    pub trait ShadowRegister {
        fn shadow_read(map: &ShadowMap) -> Option<&Self>;
        fn shadow_write(&self, map: &mut ShadowMap);
    }
...
macro_rules! registers {
    ...
    (@generate_shadow([((), $name:ident, $block:ident) $(, $($rest:tt)*)?], [$($fields:tt)*])) => {
        registers!(@generate_shadow([$($($rest)*)?], [$($fields)*]));

        #[automatically_derived]
        impl shadow::ShadowRegister for $name {
            /// This register is not using the shadow map. Therefore this always returns `None`.
            #[inline(always)]
            fn shadow_read(map: &shadow::ShadowMap) -> Option<&Self> {
                None
            }

            /// This register is not using the shadow map. Therefore this is a NOP.
            #[inline(always)]
            fn shadow_write(&self, map: &mut shadow::ShadowMap) {
                // nop
            }
        }
    };
    (@generate_shadow([((shadow), $name:ident, $block:ident) $(, $($rest:tt)*)?], [$($fields:tt)*])) => {
        registers!(@generate_shadow([$($($rest)*)?], [$($fields)* $name,]));

        #[automatically_derived]
        impl shadow::ShadowRegister for $name {
            /// Returns the value for this register that is cached in the shadow map.
            #[inline(always)]
            fn shadow_read(map: &shadow::ShadowMap) -> Option<&Self> {
                map.$block.$name.as_ref()
            }

            /// Updates the cached value for this register in the shadow map.
            #[inline(always)]
            fn shadow_write(&self, map: &mut shadow::ShadowMap) {
                map.$block.$name = Some(*self);
            }
        }

    };
    (@generate_shadow([], [$($name:tt,)*])) => {
        /// Stores cached values for this block.
        ///
        /// Refer to the [`shadow`] module for more information.
        #[allow(non_snake_case)]
        #[derive(Clone, Copy, Debug, Default)]
        pub struct ShadowBlock {
            $(
                #[doc = concat!("See [`", concat!(stringify!($name), "`]"))]
...
pub mod usb {
    ...
    registers! {
        ...
        /// This might be some status register.
        ///
        /// We never wrote to it, and librtlsdr doesn't, but for rtl_test and our small_init dump it's 0x1d and 0x1e respectively.
        /// Note that rtl_test clears fifo_reset and stall_endpoint in EPA_CTL.
        /// That indicates that the changed bit in this register shows that data is streaming.
        UNK_1: u32 = usb(0x014a) {

        };

        /// This also differs between rtl_test and small_init dump
        UNK_2: u32 = usb(0x0020) {
...
pub mod sys {
    ...
    registers! {
        ...
        DEMOD_CTL: u8 = sys(0x3000) shadow {
        ...
        ///
        DEMOD_CTL_1: u8 = sys(0x300b) shadow {
            // todo
        };

        /// IR Sensor Discontinuous Turned ON. Controlled by GPIO3
